...
5. **Optimization:** Applies educational optimization passes over the TAC (Constant Folding, Propagation, Simplification, DCE) to generate Optimized TAC.
6. **Target Code Generation:** Maps optimized TAC to an educational stack/register Virtual Machine Instruction Set (PUSH, LOAD, ADD, etc.).
7. **Virtual Machine Execution:** Runs the generated target code natively on the python-backed educational interpreter, pushing strings to output streams instead of printing instantly to emulate isolated runtime limits.
